...
import { useGetForumDetailsDataQuery } from "@/store/api/eventsDetailsApis";
...
const EventPage: React.FC = () => {
  ...
  const { data, isLoading, error } = useGetForumDetailsDataQuery(slug);

  if (isLoading)
    return <p className="text-center mt-10 text-gray-500">Loading...</p>;
  if (!data)
    return <p className="text-center mt-10 text-red-500">Forum not found.</p>;
  if (error)
    return (
  ...
  const photos = data.acf?.gallery || [];
  // const videos = data.acf?.videos || [];
  const documents = data.acf?.documents || [];
  const otherEvents = data.acf?.other_events || [];
